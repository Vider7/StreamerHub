...
- Icons are Font Awesome, bundled locally via npm (no CDN, the dashboard works offline); transport keeps light text-presentation glyphs. Reason: owner chose FA as the icon language, but solid FA playback glyphs read too heavy next to the quiet rack, so play/pause/skip stay thin (R-04, R-31).
